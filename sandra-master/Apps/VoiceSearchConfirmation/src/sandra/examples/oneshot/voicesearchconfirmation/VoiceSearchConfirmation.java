...
import sandra.examples.oneshot.voicesearchconfirmation.R;

/**
 * VoiceSearch: initiates a search query based on the words spoken by the user. 
 * It is the same as <code>VoiceSearch</code>, except that it confirms the criteria before performing the search
...
public class VoiceSearchConfirmation extends ASR {
    ...
    private static final String LOGTAG = "VOICESEARCHCONFIRMATION";
    private TTS myTts; 	
	...
	protected void onCreate(Bundle savedInstanceState) 
	{
		...
		//Set layout
		setContentView(R.layout.voicesearchconfirmation);
	...
	private void indicateSearch(String criteria) {
		...
		try {
			myTts.speak(getResources().getString(R.string.searching_prompt)+criteria, "EN");
		} catch (Exception e) {
	...
	/**
	 	Asks the user to confirm the recognized input
	 */
		...
		if(nBestList!=null){
			if(nBestList.size()>0){
				if(searchCriterion==null) {
					searchCriterion = nBestList.get(0);
					askConfirm(searchCriterion);
				}
				else {
					String confirmation = nBestList.get(0);
					if(confirmation.toLowerCase(Locale.US).equals("yes")){
						indicateSearch(searchCriterion); //Provides feedback to the user that search is going to be started
						googleText(searchCriterion);						
					}
					else{
						changeButtonAppearanceToDefault();
					}
